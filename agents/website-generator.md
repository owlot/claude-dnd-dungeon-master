...
- **Mid-scene `---` markers are not decorative — they mark audio split points.** A bare `---` line *inside* a scene's prose (as opposed to the one between two `##` scenes) means the story-teller placed it there so a mid-scene memoir anchor has somewhere to interrupt the narrator audio. When converting, emit it as `<div class="scene-break" data-audio="[N][suffix]-[slug].mp3">* * *</div>` — where `[suffix]` is `b`, `c`, `d`... counting each mid-scene break within that scene (first break after the `##` heading is `b`, the next is `c`, etc.), matching the filenames `generate-audio.py` will produce for that scene's parts. `story_to_html.py` already implements this (`PART_SUFFIXES`) — do not simplify it down to the plain scene-break div, and do not add your own `---` markers; only convert the ones already in the story. `website/js/audio.js` reads this `data-audio` attribute to wire up a continuation play button and auto-advance into it after the preceding memoir's audio finishes — dropping the attribute silently breaks that sequencing even though the page will still render fine.
